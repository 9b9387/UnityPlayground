...
public partial class TouchKit
{
	...
	private bool shouldProcessMouseInput()
	{
		...
		Debug.Log($"{_hasActiveSimulatedMultitouch} {Input.GetKey(KeyCode.A) || Input.GetKeyUp(KeyCode.A)}");
		// if enabled and alt is being held down we are simulating pinching
		if ( simulateMultitouch && ( _hasActiveSimulatedMultitouch || Input.GetKey( KeyCode.A ) || Input.GetKeyUp( KeyCode.A ) ) )
		{
			Debug.Log("xxx");
			...
			if( ( Input.GetKeyUp( KeyCode.A ) || Input.GetMouseButtonUp(0)) && _hasActiveSimulatedMultitouch )
			{
